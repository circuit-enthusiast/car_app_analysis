...
CODE_CSV = "code_analysis.csv"


def handle_score_code_analysis(dataframe: pd.DataFrame) -> dict[str, int]:
...
def load_code_counts(base_path: Path = PROJECT_ROOT) -> dict[str, dict[str, int]]:
    counts: dict[str, dict[str, int]] = {}
    for app in available_apps(base_path):
        csv_path = base_path / app / CODE_CSV
        if not csv_path.exists():
            continue
        dataframe = pd.read_csv(csv_path)
        counts[Path(app).name] = handle_score_code_analysis(dataframe)
    return counts


def save_code_bar_chart(
    counts: Mapping[str, Mapping[str, int]],
    output_path: Path,
) -> Path:
    if not counts:
        raise ValueError("No code counts provided")
...
def save_code_summary_csv(counts: Mapping[str, Mapping[str, int]], output_path: Path) -> Path:
    if not counts:
        raise ValueError("No code counts provided")
...
def generate_code_report(base_path: Path = PROJECT_ROOT) -> Path | None:
    counts = load_code_counts(base_path)
    if not counts:
        return None
    output_path = CODE_DIR / "code_analysis_counts.png"
    output_path.parent.mkdir(parents=True, exist_ok=True)
    if output_path.exists():
        output_path.unlink()

    # save chart
    chart_path = save_code_bar_chart(counts, output_path)

    # save summary CSV alongside the chart
    csv_path = CODE_DIR / "code_analysis_summary.csv"
    if csv_path.exists():
        csv_path.unlink()
    save_code_summary_csv(counts, csv_path)

    return chart_path


if __name__ == "__main__":
    path = generate_code_report()
    if path:
        print(f"Saved code analysis chart to {path}")
        print(f"Saved summary CSV to {CODE_DIR / 'code_analysis_summary.csv'}")
        # also print a brief severity table for CLI users
        counts = load_code_counts()
        for m, sevmap in counts.items():
            print(f"{m}:")
            for sev, c in sorted(sevmap.items(), key=lambda x: (-x[1], x[0])):
                print(f"  {sev}: {c}")
    else:
        print("No code analysis data available to plot")
